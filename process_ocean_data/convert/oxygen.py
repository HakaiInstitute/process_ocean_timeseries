...
def  O2ctoO2p(O2conc,T,S,P=0):
    ...
    return O2conc * (xO2 * (1013.25-pH2Osat)) / (TCorr * Scorr) *np.exp(Vm *P /(R * (T+273.15)))
...
def O2ctoO2s(O2conc,T,S,P=0,p_atm=1013.25):
    ...
    return O2conc*100/(TCorr*Scorr)/(p_atm-pH2Osat)*(1013.25-pH2Osat)*np.exp(Vm*P/(R*(T+273.15)))
...
def O2ptoO2c(pO2,T,S,P=0):
    ...
    return pO2/(xO2*(1013.25-pH2Osat))*(TCorr*Scorr)/np.exp(Vm*P/(R*(T+273.15)))
...
def O2stoO2c(O2sat,T,S,P=0,p_atm=1013.25):
    ...
    return O2sat/100*(TCorr*Scorr)*(p_atm-pH2Osat)/(1013.25-pH2Osat)/np.exp(Vm*P/(R*(T+273.15)))
