...
def get_mag_dec_from_nrcan(time, lat, lon):
    ...
    url = (
        "https://www.geomag.nrcan.gc.ca/calc/mdcal-r-en.php?date="
        + date
        + "&latitude="
        + lat_str
        + "&latitude_direction="
    ...
    magnetic_string = re.findall(
        r"Magnetic declination: \d+\°[\s\d\.]+.\s\w+", soup.prettify()
    )
    magnetic_declination = re.split(r"[:\°'']", magnetic_string[0])

    # Get Magnetic Annual Rate
    annual_rate_string = re.findall(
        r"Annual Change \(minutes\/year\)\:[\s\d\.]+\'\/y\s\w+", soup.prettify()
    )
    annual_rate = re.split(r"\:|\'\/y", annual_rate_string[0])

    magnetic_declination_value = (
        float(magnetic_declination[1]) + float(magnetic_declination[2]) / 60
    )
...
def trilateration_from_utm(distances_to_station, stations_coordinates):
    ...
    W = [
        ((n_stations - 1) * distances_sum) / (distances_sum - w)
