...
def btl(file_path,output='xarray'):
    with open(file_path) as f:
        ...
        if header['variables']:
            ...
            header['variables'] = add_seabird_vocabulary({var[0].lower()+var[1:]:{} for var in header['bottle_columns']})
    ...
    for var in ds:
        ...
        if var_std in ds:
            ...
            ds[var_std].attrs['cell_method'] = f"scan: standard_deviation (previous {n_scan_per_bottle} scans)"
            # TODO confirm that seabird uses the previous records from this timestamp
        if var not in ['time','bottle']:
            ds[var].attrs['cell_method'] = f"scan: mean (previous {n_scan_per_bottle} scans)"
...
def parse_seabird_file_header(f):
    ...
    def read_asterisk_line(line):
        if " = " in line:
            ...
            instrument_type = re.search('\* Sea-Bird (.*) Data File\:|\* SBE (.*)',line).groups()
            header['instrument_type'] += "".join([item for item in instrument_type if item])
        elif re.match(r"\* Software version .*",line, re.IGNORECASE):
            header['software_version'] = re.search('\* Software version (.*)',line,re.IGNORECASE)[1]
        else:
            unknown_line(line)
    ...
    while "*END*"  not in line and line.startswith(('*','#')):
        ...
        if re.match('^\*\s*$',line) or '*END*' in line or re.match('^\s*Bottle .*',line):
            continue
